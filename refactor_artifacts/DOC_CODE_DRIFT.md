...
| D-02 ✅**机制已证实 / 影响判断被推翻** | README 让 agent 读 `engines_consensus` / `consensus_basis` 判断池是否降级 | 实测：`DHOLE_DEFAULT_ENGINES=bing` 时 `_configured_default_backends()` 确实收敛为 `['bing']`，但 `_family_universe(None, [])` 前后都是 `(4, 0, 'single_family')` —— **分母 4 纹丝不动**（机制为真）。**但"自建小池会被读成池降级"不成立**：实测小池得到 `basis=single_family`，安全信号本来就生效；按"让分母跟随启用池"去修会产出 **"1 of 1"**，而那正是 docstring 明确要避免的字符串 | 机制为真、**影响与建议均被推翻 → 不是缺陷，是设计取舍** | **不改代码**；若动，正确方向是文档说明分母含义。详见 `KNOWN_BUGS.md` KB-2 的更正 |
