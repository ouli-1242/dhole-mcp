...
## D-15 KB-2 结案为"设计取舍"，不修代码

- **背景**：我先前把 D-02 记成缺陷并建议"让分母跟随实际启用的池"。用户问"如果修了会是升级吗"，
  我为此去实测"修了之后会变成什么"，结果**自己的判据被推翻**。
- **实测**（真实 `EngineReport` 构造各场景调 `_family_universe()`）：
  只看 bing → `basis=single_family`（不是 `partial_pool`）；`partial_pool` 需真有家族被 preempted，
  `degraded_pool` 需真有引擎被 blocked，判据都取自实际轮次、与分母用哪个池无关；
  6 个默认引擎映射到 **4 个索引家族**（bing←bing/duckduckgo/yahoo）。反向看，
  `分母=1` 是能出现的（家族被 preempted 时），那时输出 **"1 of 1"** ——
  而 `_family_universe` 的 docstring 明确说这正是它要避免的字符串。
- **选定**：**不改代码**。按原建议修会产出 "1 of 1"，比现状更糟。正确方向若仍要动，
  是在文档里说明分母的含义（"默认池的索引家族数"）—— 那属于改 `tools/list`，仍需产品决策。
- **影响**：`KNOWN_BUGS.md` KB-2 与 `DOC_CODE_DRIFT.md` D-02 都已改写为"机制为真、影响与建议被推翻"。
  **这是本轮第二次由实测推翻我自己的结论**（第一次是 KB-11 的假绿范围）。

## D-16 版本折回 14.6，而不是升到 14.8

- **背景**：用户指出"现在应该还是 14.6 版本"。核对：本地 tag 只到 `v14.6`，
  **没有 `v14.7`**；14.7 那批提交比 `origin/master` 领先 22 个且从未推送。
  也就是说 14.7 从未发布，不存在需要兼容的已发布产物。
- **选定**：`__version__` 14.7 → **14.6**；CHANGELOG 的 `[14.7]` 段并入 `[14.6]` 段
  （两个 `### 测试` 合并为一、正文里的版本引用改写、新增 `### 修复（复验后追加）` 记 KB-1/KB-7）。
- **理由**：为一个从未发布的版本号付一次 minor 位没有意义；而把未发布的批次塞进已定的版本号里，
  版本号才继续是"用户装到的那一版"的可靠标识。
- **诚实边界**：
  1. tag 只是**本地**证据。PyPI 上到底有没有 14.7，我**无法在不联网的情况下确认**——
     若不巧已经发布过，那这次折叠会让新内容与已发布的 14.6 同名。
  2. 改 `__version__` 会影响 MCP 握手暴露的 `serverInfo.version` 与 `dhole -v`，**是可观察行为**，
     所以它只落在 `release/14.6` 分支，不进重构分支。
  3. `tests/test_tool_descriptions.py` 里那句 `# 14.7: 550 -> 860` 注释、README 的 token 表括注
     里各有一处旧版本号，一并改掉（只改版本号，没有借机把文档对齐代码）。

## D-17 用 Docker 容器补 POSIX 实证，并把网络请求记账

- **背景**：KB-6 的验证链此前缺最后一环：本机 Windows 无法验证 POSIX 权限位。
  用户启动 Docker 并明确说"KB-6 应该也能测试了"，缺口有办法补上。
- **选定**：拉 `python:3.12-slim`（179MB，一次；按任务要求记账），
  用 `git archive` 构造修复前/后两棵树，跑同一个 harness 与同一份测试文件。
- **理由**：这是"证据 vs 断言"的分界。补测前我在两份文档里都写明"POSIX 真实 mode 未实测"；
  补测后它变成了实测数字（修复前 0644/0755，修复后 0600/0700），并且修复前的树上
