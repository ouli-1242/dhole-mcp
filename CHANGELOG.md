...
一次以「让静默降级可被观测、把说得太满的地方改准」为主的版本。测试 755 → 909，
仍然全部离线（默认运行零网络）。
...
- **`engines_consensus` 不再在降级池上伪装成「全员一致」。** 分母原先是「本轮实际
  ...
  `1 of 1 (no corroboration)`。**默认 5 引擎池只有 3 个索引家族**（bing/duckduckgo/
  yahoo 同一索引），所以 `3 of 3` 才是满分，此前文档里的 `3 of 5` 是不可能出现的字符串。
  本机实测 10/10 次搜索都是这个形态。
...
- **三个状态文件的落点改为惰性求值。** `circuit_breaker.json` / `search_feedback.json`
  ...
  目录，并有一组测试钉住「必须由 paths.py 派生且是惰性的」这条约定。实测复验：全量 909
  个用例跑完，真实 `~/.dhole` 下所有文件的 size/mtime 零变化。
...
- **重排模型支持发布方 sha256 真实性校验。** 原实现是「下载完算自己刚下的字节的哈希再
  ...
  原因），但**三个模型的摘要值当前全部留空**：这台机器上 HF API 返 502、hf-mirror API 返
  403，拿不到权威值——没有编造数字来伪造信任，并有测试断言注册表里不许出现未核对的摘要。
